...
for fold, (train_indices, test_indices) in enumerate(kf.split(dataset)):
    ...
    for epoch in range(1, 26):
        ...
        print(f'Fold: {fold+1}, Epoch: {epoch:03d}, Train Loss: {train_loss:.4f}, Test Loss: {test_loss:.4f}, Accuracy: {accuracy:.4f}, AUC: {auc_score:.4f}, AUPR: {aupr_score:.4f}')
        print(f'Confusion Matrix:\n{conf_matrix}')

    all_auc_scores.append(auc_score)
    all_accuracies.append(accuracy)

print(f'Mean AUC: {np.mean(all_auc_scores):.4f}, Std AUC: {np.std(all_auc_scores):.4f}')
print(f'Mean Accuracy: {np.mean(all_accuracies):.4f}, Std Accuracy: {np.std(all_accuracies):.4f}')
print(f'AUC: {all_auc_scores}')
print(f'Accuracy: {all_accuracies}')